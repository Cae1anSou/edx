...
> Historical note (split branch): As of 2026-02-28, top-level `lms/` and `cms/` directories were removed from this branch. Older plan items may still reference those paths as historical context.
...
- [x] Run full frontend build and runtime smoke in current environment (build PASS).
...
| Cannot perform full LMS/CMS page migration in this repo snapshot | 1 | Historical note: at that time `lms/` and `cms/` only contained `static/` assets; later they were removed from this split branch on 2026-02-28 |
...
- [x] Re-run frontend build in current environment (PASS).
- **Status:** in_progress
...
- [x] Run frontend build validation in current environment (PASS).
- **Status:** in_progress
...
- [x] Frontend build/runtime verification in current environment (PASS).
- **Status:** in_progress
...
- [x] Re-run parity and backend tests.
- [x] Frontend build/runtime validation in current environment (PASS).
- **Status:** in_progress
...
### Phase 27: Release Readiness Validation (Go/No-Go)
- [ ] Run backend/frontend regression tests (unit/integration/contract). (frontend and contract checks completed in current environment; backend mvn still environment-dependent)
- [x] Expand core E2E to cover auth, write operations, and error branches.
- [x] Execute pre-production style integration + migration validation checklist.
- [x] Fix contract tooling false negatives (`/api/studio/v1/*` class-level mapping parsing).
- [x] Fix template normalization for dangling `${suffix|query|problemQuery` paths.
- [x] Move E2E TS from `@ts-nocheck` to typed compile pipeline (`tsconfig.e2e.json` + npm scripts).
- [ ] Produce launch recommendation with explicit blockers/risk level.
- **Status:** in_progress
