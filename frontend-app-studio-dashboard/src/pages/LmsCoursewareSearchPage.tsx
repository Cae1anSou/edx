...
function parseCourse(pathname: string): string {
  ...
  const stop = tail.findIndex((seg) => ['courseware-search', 'courseware', 'about', 'progress', 'dates'].includes(seg));
  const keyParts = stop >= 0 ? tail.slice(0, stop) : tail;
  return decodeURIComponent(keyParts.join('/'));
}

export function LmsCoursewareSearchPage() {
  const location = useLocation();
  const courseKey = useMemo(() => parseCourse(location.pathname), [location.pathname]);

  return (
    <main className="container legacy-v1-shell legacy-v1-generic legacy-v1-lms-courseware-search">
      <section className="legacy-v1-mast">
        <div>
          <h1 className="legacy-v1-title-with-sub">
            <span className="legacy-v1-subtitle">LMS Course</span>
            <span>Courseware Search</span>
          </h1>
          <p>Legacy LMS courseware search settings page shell.</p>
        </div>
      </section>
